...
func (s *SubscriptionService) Create(ctx context.Context, tenantID primitive.ObjectID, plan models.SubscriptionPlan) (*models.Subscription, error) {
	if plan == "" {
...
func (s *SubscriptionService) UpdatePlan(ctx context.Context, tenantID primitive.ObjectID, plan models.SubscriptionPlan) error {
	if !validPlans[plan] {
	...
	now := time.Now()
	return s.repo.UpdatePlan(ctx, tenantID, plan, now, now.AddDate(0, 0, subscriptionPeriodDays))
}
...
func (s *SubscriptionService) Cancel(ctx context.Context, tenantID primitive.ObjectID) error {
	if _, err := s.repo.FindByTenantID(ctx, tenantID); err != nil {
		if err == mongo.ErrNoDocuments {
			return apierr.NotFound("tenant has no subscription")
		}
	...
	return s.repo.UpdateStatus(ctx, tenantID, models.SubscriptionCanceled)
}
