...
func (r *SubscriptionRepo) UpdatePlan(ctx context.Context, tenantID primitive.ObjectID, plan models.SubscriptionPlan, periodStart, periodEnd time.Time) error {
	_, err := r.col.UpdateOne(ctx, bson.M{"tenant_id": tenantID}, bson.M{"$set": bson.M{
		"plan":                 plan,
...
func (r *SubscriptionRepo) UpdateStatus(ctx context.Context, tenantID primitive.ObjectID, status models.SubscriptionStatus) error {
	set := bson.M{
		"status":     status,
		"updated_at": time.Now(),
	}
